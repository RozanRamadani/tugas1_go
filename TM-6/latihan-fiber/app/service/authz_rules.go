...
func CanAccessUser(
	...
	// Jalur 1: Kepemilikan sendiri (Paling cepat & paling sering)
	if current.UserID == targetID {
		return true
	}

	// Jalur 2: Cek permission untuk data orang lain
...
func ValidateAssignRole(
	...
	// Aturan Keamanan: Admin tidak boleh mengubah role dirinya sendiri
	if current.UserID == targetID {
